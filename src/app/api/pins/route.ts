...
// Fails open (treats content as clean) on moderation errors — an
// anonymous fan-map isn't high-stakes enough to block every pin drop over
// an API hiccup; admins can still delete anything that slips through (see
// Phase 6c). Uses forced tool-calling for reliable structured output,
// since the Messages API has no dedicated JSON-mode like some providers.
async function moderateContent(title: string, description: string | null): Promise<ModerationResult> {
  ...
  try {
    ...
    console.error("Moderation check failed, allowing pin through:", err);
    return { flagged: false };
  }
}
